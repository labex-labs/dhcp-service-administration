# Администрирование службы DHCP

**Языки:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/ru/courses/dhcp-service-administration">
    <img src="https://course-cover.labex.io/dhcp-service-administration.png?lang=ru" alt="Администрирование службы DHCP">
  </a>
</p>

Изучите администрирование службы DHCP. Этот модуль охватывает установку сервера ISC DHCP, настройку области подсети, резервирование фиксированных IP-адресов, анализ аренды и устранение неполадок DHCP. Вы освоите эти важные навыки Linux с помощью практических лабораторных работ и реальных задач.

[Начать курс на LabEx](https://labex.io/ru/courses/dhcp-service-administration)

## Упражнения

|   Индекс | Название                                 | Сложность   | Практика                                                                                                                                       |
|----------|------------------------------------------|-------------|------------------------------------------------------------------------------------------------------------------------------------------------|
|        1 | Установка ISC DHCP-сервера               | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/installing-isc-dhcp-server-636542?course=dhcp-service-administration'>Начать лабораторию</a> |
|        2 | Настройка области подсети (Subnet Scope) | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/configuring-a-subnet-scope-636540?course=dhcp-service-administration'>Начать лабораторию</a> |
|        3 | Резервирование фиксированных IP-адресов  | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/fixed-ip-reservations-636541?course=dhcp-service-administration'>Начать лабораторию</a>      |
|        4 | Анализ арендованных IP-адресов (Leases)  | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/analyzing-leases-636539?course=dhcp-service-administration'>Начать лабораторию</a>           |
|        5 | Устранение неполадок DHCP                | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/troubleshooting-dhcp-636543?course=dhcp-service-administration'>Начать лабораторию</a>       |
|        6 | Настройка офисной сети                   | Средний     | <a target='_blank' href='https://labex.io/ru/labs/office-network-provisioning-636733?course=dhcp-service-administration'>Начать испытание</a>  |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

