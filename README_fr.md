# Administration du Service DHCP

**Langues:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/fr/courses/dhcp-service-administration">
    <img src="https://course-cover.labex.io/dhcp-service-administration.png?lang=fr" alt="Administration du Service DHCP">
  </a>
</p>

Apprenez l'administration du service DHCP. Ce module couvre l'installation du serveur DHCP ISC, la configuration d'une portée de sous-réseau, les réservations d'adresses IP fixes, l'analyse des baux (leases), et le dépannage DHCP. Vous maîtriserez ces compétences Linux essentielles grâce à des laboratoires pratiques et des défis concrets.

[Commencer le cours sur LabEx](https://labex.io/fr/courses/dhcp-service-administration)

## Exercices

|   Index | Nom                                        | Difficulté    | Pratique                                                                                                                                       |
|---------|--------------------------------------------|---------------|------------------------------------------------------------------------------------------------------------------------------------------------|
|       1 | Installation du serveur ISC DHCP           | Débutant      | <a target='_blank' href='https://labex.io/fr/labs/installing-isc-dhcp-server-636542?course=dhcp-service-administration'>Commencer le lab</a>   |
|       2 | Configuration d'une étendue de sous-réseau | Débutant      | <a target='_blank' href='https://labex.io/fr/labs/configuring-a-subnet-scope-636540?course=dhcp-service-administration'>Commencer le lab</a>   |
|       3 | Réservations d'adresses IP fixes           | Débutant      | <a target='_blank' href='https://labex.io/fr/labs/fixed-ip-reservations-636541?course=dhcp-service-administration'>Commencer le lab</a>        |
|       4 | Analyse des baux DHCP                      | Débutant      | <a target='_blank' href='https://labex.io/fr/labs/analyzing-leases-636539?course=dhcp-service-administration'>Commencer le lab</a>             |
|       5 | Dépannage DHCP                             | Débutant      | <a target='_blank' href='https://labex.io/fr/labs/troubleshooting-dhcp-636543?course=dhcp-service-administration'>Commencer le lab</a>         |
|       6 | Provisionnement du réseau de bureau        | Intermédiaire | <a target='_blank' href='https://labex.io/fr/labs/office-network-provisioning-636733?course=dhcp-service-administration'>Commencer le défi</a> |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

